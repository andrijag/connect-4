...
class Game(Subject):
    ...
    def drop(self, i):
        if self._legal_move(i):
            self.player.drop(self.board, i)
            if self.winning_move(i):
                self._end_game()
            else:
                self._next_turn()
    ...
    def _legal_move(self, i):
        return not self.game_over and not self.board[i][-1]

    def winning_move(self, i):
        for j in reversed(range(self.board.n_rows)):
            if self.board[i][j]:
                print(j)
...
class Player:
    ...
    def drop(self, board, i):
        board.stack(i, self.id_)


class Board:
    def __init__(self, n_rows, n_columns):
        self._board = [[0 for _ in range(n_rows)] for _ in range(n_columns)]
        self.n_rows = n_rows
